...
// Initialize mobile menu functionality
    ...
    // 1. Create the Top Bar for Search and Customer Service
    const topBar = document.createElement('div');
    topBar.style.cssText = "background: rgba(0,0,0,0.8); padding: 10px 5%; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(0,255,204,0.2); position: relative; z-index: 1001; font-family: sans-serif;";

    // Generate a random session/customer number
    const randomID = Math.floor(100000 + Math.random() * 900000);

    topBar.innerHTML = `
        <div style="color: #adb5bd; font-size: 12px; letter-spacing: 1px;">
            <i class="fas fa-headset" style="color: #00ffcc; margin-right: 5px;"></i>
            <a href="contact.html" style="color: #adb5bd; text-decoration: none; text-transform: uppercase;">Customer Service</a>
            <span style="margin-left: 20px; border-left: 1px solid #333; padding-left: 20px;">ID: <span style="color: #00ffcc;">#${randomID}</span></span>
        </div>
        <div style="position: relative; width: 250px;">
            <input type="text" placeholder="Search AI Scents..." style="width: 100%; background: #111; border: 1px solid #00ffcc; color: #fff; padding: 6px 15px; border-radius: 20px; font-size: 13px; outline: none;">
            <i class="fas fa-search" style="position: absolute; right: 12px; top: 8px; color: #00ffcc; cursor: pointer;"></i>
        </div>
    `;

    // Inject the top bar before the Navigation
    const nav = document.querySelector('nav');
    if (nav) {
        nav.parentNode.insertBefore(topBar, nav);
    }

    // 2. Add the Random Number Subheading to the Hero Section
    const heroTitle = document.querySelector('.hero-title');
    if (heroTitle) {
        const subHeading = document.createElement('p');
        subHeading.style.cssText = "color: #00ffcc; font-family: monospace; font-size: 14px; margin-top: -10px; margin-bottom: 20px; letter-spacing: 4px; text-transform: uppercase; opacity: 0.8;";
        subHeading.innerHTML = `Nexus_Protocol_Node_${randomID} | Status: Active`;
        
        // Insert right after the main title
        heroTitle.insertAdjacentElement('afterend', subHeading);
    }
});
